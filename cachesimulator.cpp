...
Cache::Block* CacheSimulator::find_block(uint32_t address) const {
  ...
  for (Cache::Block* block : _cache->get_blocks_in_set(extract_index(address, _cache->get_config()))) {
...
Cache::Block* CacheSimulator::bring_block_into_cache(uint32_t address) const {
  ...
  for (Cache::Block* block : _cache->get_blocks_in_set(extract_index(address, _cache->get_config()))) {
    // found an invalid block, we can just skip to end
    if (!block->is_valid()) {
      lru = block;
      break;
    }

    // compare and try to find the lru
    if (lru == NULL || block->get_last_used_time() < lru->get_last_used_time()) {
      lru = block;
    }
  }
